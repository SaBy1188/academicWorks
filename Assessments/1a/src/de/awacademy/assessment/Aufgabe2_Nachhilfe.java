...
public class Aufgabe2_Nachhilfe {
    ...
    public static List<Product> findByCategory(List<Product> products, Category category) {
        ...
        */
        ...
        List<Product> sortedByCategoryList = new ArrayList<>();

        /* Alternative:

        for (Product product : products) {
            if (product.getCategory().equals(category)) {
                sortedByCategoryList.add(product);
            }
        }
        */
        ...
        for (int i = 0; i < products.size(); i++) {
            ...
            if (product.getCategory().equals(category)) {           // .getCategory ruft Datentyp Category auf und sucht dort ob .equals() bereits andere mit selbiger vorhanden sind
    ...
    private static int countByCategory(List<Product> products, Category category) {
        ...
        /*
            ...
            Die ursprüngliche Liste darf dabei nicht verändert werden.
            => ArrayList iteriert von Index 0 bis Ende durch, daher bleibt die Reihenfolge gegeben.
        */

        // return (int) products.stream().filter(product -> product.getCategory().equals(category)).count();
...
/*      // Liste aus ENUMs ist möglich! Alle vorhandenen ENUMs aus der Liste können einer neuen hinzugefügt werden
        // Dies verhält sich genau wie andere Klassen (bsp. Integer, String usw.)
        List<Category> enumList = new ArrayList<>();
        enumList.add(Category.FOOD);
        // Prüfen, ob ein Element in einer Liste vorhanden ist
        boolean foodIsInList = enumList.contains(Category.FOOD);

        public static boolean methodeENUM(Category category) {
            if (enumList.contains(category)) {
                return true;
            } return false;
        }*/
